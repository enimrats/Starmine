...
actor DandanplayClient {
    ...
    private let danmakuProxyURL = URL(string: "https://nipaplay.aimes-soft.com/danmaku_proxy.php")!
    ...
    func searchAnime(keyword: String) async throws -> [AnimeSearchResult] {
        ...
        return rawResults.compactMap { item in
            ...
            let imageURL = (item["imageUrl"] as? String).flatMap(URL.init(string:))
            return AnimeSearchResult(
                id: animeID,
                title: (item["animeTitle"] as? String) ?? (item["title"] as? String) ?? "未命名条目",
    ...
    func loadEpisodes(for animeID: Int) async throws -> [AnimeEpisode] {
        let path = "/api/v2/bangumi/\(animeID)"
        let data = try await requestData(apiPath: path)
        ...
        if
            let bangumi = payload["bangumi"] as? [String: Any],
        ...
        return rawEpisodes.compactMap { item in
            ...
            return AnimeEpisode(
                ...
                title: (item["episodeTitle"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty
                ?? "第 \(item["episodeNumber"] as? Int ?? 0) 话"
            )
        }
    }
    
    func loadDanmaku(episodeID: Int, chConvert: Int = 0) async throws -> [DanmakuComment] {
        ...
        do {
            ...
            let data = try await requestData(apiPath: apiPath, query: query, useProxy: true)
            return try parseDanmaku(from: data)
        }
    }
    ...
    private func parseDanmaku(from data: Data) throws -> [DanmakuComment] {
        ...
        for item in rawComments {
            ...
            let parts = pValue.split(separator: ",", omittingEmptySubsequences: false)
    ...
    private func requestData(
        ...
        let targetURL: URL
        if useProxy {
            var proxyComponents = URLComponents(url: danmakuProxyURL, resolvingAgainstBaseURL: false)!
        ...
        request.setValue(Self.signature(timestamp: timestamp, apiPath: apiPath, appSecret: secret), forHTTPHeaderField: "X-Signature")
        ...
        guard (200 ..< 300).contains(httpResponse.statusCode) else {
            ...
            let message = httpResponse.value(forHTTPHeaderField: "X-Error-Message")
    ...
    private func resolvedAppSecret() async throws -> String {
        ...
        for server in secretServers {
            do {
                var request = URLRequest(url: server.appendingPathComponent("nipaplay.php"))
    ...
    private static func signature(timestamp: Int, apiPath: String, appSecret: String) -> String {
        let raw = Data((Self.appID + String(timestamp) + apiPath + appSecret).utf8)
    ...
    static func decodeEncryptedAppSecret(_ input: String) -> String {
        ...
        return String(digitMapped.map { character in
