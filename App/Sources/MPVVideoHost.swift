...
final class MPVVideoHostView: NSView {
    ...
    func notifyReady() {
        let pointer = Int64(Int(bitPattern: Unmanaged.passUnretained(metalLayer).toOpaque()))
        onReady?(pointer)
    }
    ...
    private func syncMetalLayerSize() -> Bool {
        ...
        let scale = window?.backingScaleFactor ?? window?.screen?.backingScaleFactor ?? NSScreen.main?.backingScaleFactor ?? 1
        let viewportChanged = size != lastViewportSize || scale != lastViewportScale
...
final class MPVVideoHostView: UIView {
    ...
    func notifyReady() {
        let pointer = Int64(Int(bitPattern: Unmanaged.passUnretained(self).toOpaque()))
        onReady?(pointer)
    }
}
#endif
